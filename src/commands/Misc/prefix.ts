...
export default class extends Command {
	...
	public async run(message: Message, args: Args) {
		...
		const guildSettings = await getCustomRepository(GuildSettingRepository).get(message.guild!);
		...
		await getCustomRepository(GuildSettingRepository).save(guildSettings);
