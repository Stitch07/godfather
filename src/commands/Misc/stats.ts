...
export default class extends GodfatherCommand {
  ...
  private async buildEmbed(message: Message) {
    ...
    return new MessageEmbed()
      ...
      .setDescription(
        `To add ${this.context.client.user!.username} to your server, use the \`${Array.isArray(prefix) ? prefix[0] : prefix}invite\` command.`
      )
      .addField(
        'Connected To',
        [`**Servers**: ${generalStatistics.guilds}`, `**Users**: ${generalStatistics.members}`, `**Channels**: ${generalStatistics.channels}`].join(
          '\n'
        ),
        true
      )
