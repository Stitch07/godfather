...
export default class Game {
	...
	public async startDay() {
		...
		for (const player of this.players) {
			// clear visitors
			player.visitors = [];
			if (player.isAlive && player.role instanceof ActionRole && player.role.canUseAction().check && (cast<ActionRole>(player.role).actionPhase & Phase.Day) === Phase.Day) {
	...
	public async startNight() {
		...
		for (const player of this.players.filter(
			(player): boolean => fauxAlive(player) && (player.role instanceof ActionRole) && player.role!.canUseAction().check && cast<ActionRole>(player.role!).actionPhase === Phase.Night
		)) {
