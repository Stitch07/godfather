...
export default class VoteManager extends Map<string, VoteProxy> {
	...
	public unvote(voter: Player) {
		for (const [target, votes] of this) {
			...
			votes.splice(
				votes.findIndex((vote) => vote.by === voter),
				1
			);
	...
	public show({ header = 'Vote Count', codeblock = false }): string {
		...
		const sortedVotes = Array.from(this.entries())
			.filter((votes) => votes[0] !== NotVoting)
		...
		for (const [targetID, votes] of sortedVotes) {
			...
			if (!target) continue;
			voteText.push(
				`${target.user.username} (${votes.count()}): ${votes.map(voterMapping).join(', ')} ${
					votes.count() >= this.game.majorityVotes ? '(Hammered)' : ''
				}`
			);
		}
